library(dplyr)
HCA_X_train <- read.table("./train/X_train.txt")

HCA_y_train <- read.table("./train/y_train.txt")

HCA_subject_train <- read.table("./train/subject_train.txt")
...
HCA_X_test <- read.table("./test/X_test.txt")

HCA_y_test <- read.table("./test/y_test.txt")

HCA_subject_test <- read.table("./test/subject_test.txt")
...
colnames(HCA_tidy) <- c("tBodyAcc_mean_X","tBodyAcc_mean_Y","tBodyAcc_mean_Z",
                        ...
                        "y_label","subject_id")
useCol<-c(1,2)
for (i in 3:length(HCA_tidy)){
  
  if(length(grep('mean',names(HCA_tidy)[i]))==1){        
    
    useCol<-append(useCol,i)        
    
  }else if(length(grep('std',names(HCA_tidy)[i]))==1){
...
write.table(HCA_tidy,"./HCA_tidy.txt",sep=" ",row.names=FALSE)